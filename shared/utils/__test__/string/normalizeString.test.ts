...
describe('normalizeString', () => {
  ...
  it('deve remover espaços extras no início e no fim', () => {
    expect(normalizeString('   espaço   no início e fim   ')).toBe('espaco no inicio e fim')
  })

  it('deve substituir múltiplos espaços por um único', () => {
    expect(normalizeString('muitos     espaços   no    meio')).toBe('muitos espacos no meio')
  })
  ...
  it('deve combinar tudo junto', () => {
    expect(normalizeString('   Éxêmplô   DÊ   Texto   ')).toBe('exemplo de texto')
  })
})
